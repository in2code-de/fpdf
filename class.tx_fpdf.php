...
class PDF extends FPDI
{
     ...
     * @param    float    y: the y position of the bookmark destination in the current page. -1 means the current
    ...
    function BookmarkUTF8($txt, $level = 0, $y = 0)
    {
        if (!function_exists('iconv')) {
            $this->Error('iconv extension not available');
        }
        $txt = iconv('UTF-8', 'UTF-16BE', $txt);
        if ($txt === false) {
            $this->Error('The string could not be converted');
        }
